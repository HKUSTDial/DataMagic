...
**完整流程**：你有一份数据，需要做成一段完整的汇报视频。上传后 AI 自动规划场景、推荐图表并生成旁白，你审阅确认即可导出。适合业务汇报、研究展示，以及向团队或领导呈现分析结论。

**快速生成**：同样是完整视频，但需要更快出结果。AI 依然负责规划和旁白，渲染速度更快。适合周期性报告或时间紧的场合。

**Single Chart（单图表模式）**：只需要一张图来说明一个数据洞察。粘贴数据，几秒内生成带动效的图表，直接插入 PPT 或发到社交媒体。
